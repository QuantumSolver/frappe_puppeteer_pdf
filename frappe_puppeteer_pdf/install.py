...
@filelock("frappe_puppeteer_pdf_chromium_setup", timeout=1, is_global=True)
def setup_chromium():
...
class PuppeteerPDFGenerator:
    """Simple class to hold executable paths - similar to original FrappePDFGenerator"""
...
def find_or_download_chromium_executable():
    ...
    executable_name = PuppeteerPDFGenerator.EXECUTABLE_PATHS.get(platform_name)
...
def download_chromium():
    ...
    try:
        ...
        executable_path = PuppeteerPDFGenerator.EXECUTABLE_PATHS[
...
def set_pdf_generator_option(
    ...
    if generator_name in options and action == "add":
        return
    ...
    if action == "add":
        ...
        if generator_name in options:
            options.remove(generator_name)
